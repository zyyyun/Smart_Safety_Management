...
fun TbmWorkerScreen(
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        if (sessions.isEmpty()) {
            ...
            if (session == null) {
                Text("선택된 TBM 세션이 없습니다.", fontSize = 14.sp, color = Color.Gray)
            } else {
                Card(modifier = Modifier.fillMaxWidth().padding(bottom = 12.dp)) {
                    Column(modifier = Modifier.padding(12.dp)) {
                        Text(session.workScope, fontWeight = FontWeight.Bold, fontSize = 18.sp)
                        Text("OPS: ${session.workType}", fontSize = 13.sp, color = Color.Gray)
                        Text("리더: ${session.leaderUserId}", fontSize = 13.sp, color = Color.Gray)
                        Text("예상 종료: ${formatTimeShort(session.expectedEndAt)}", fontSize = 13.sp, color = Color.Gray)
                        if (sessionEnded) Text("종료됨: ${formatTimeShort(session.endedAt ?: "")}", color = Color.Gray)
                    }
                }
                ...
                Text("자가 항목", fontWeight = FontWeight.SemiBold, fontSize = 16.sp)
