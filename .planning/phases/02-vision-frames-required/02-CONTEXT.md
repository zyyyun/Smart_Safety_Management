...
- **D-06: buffer 는 프로세스 재시작 시 자동 reset (휘발 메모리)**
  ...
  - **D-06 의 "1프로세스" 의미 = 데몬 모드 (`python main.py`) 무한 cycle**.
    `--once-detect` 반복은 매 cycle 별도 프로세스 → 모듈 dict fresh → frames_required
    누적 검증 불가능. 운영 환경 + 검증 환경 모두 데몬 모드 사용 (2026-05-07 6 cycle
    실측에서 발견 → CONTEXT 명시).
