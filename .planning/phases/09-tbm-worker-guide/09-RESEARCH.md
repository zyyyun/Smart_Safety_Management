...
1. **C1 결정 — `tbm-started` 가 notifications row insert 해야 하는가?**
   ...
   - **권장:** v1.0 = 모두 push-only (단순, Phase 8 일관). v1.1 에서 TBM 알림 history Activity 추가 시 insert.
...
4. **데모 시연용 cron 시각 단축 — 09:00 시작 → 09:15 expected → 09:45 cron?**
   ...
   - **권장:** 데모 시 manual 시드 SQL 또는 expected_end_at default UX 가 "1분 후" 옵션 (planner UX 결정).
...
5. **Edge Function `tbm-end` 가 manager 의 leader_user_id 본인 가드 — bypass?**
   ...
   - **권장:** v1.0 한정 leader 본인만. v1.1 group 내 manager 누구나 종료 가능 (workspace ownership).
