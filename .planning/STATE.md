...
Status: Phase 8 IN PROGRESS — 08-01 ✓ COMPLETE (snapshot.capture_rtsp + drift_test 패턴). 다음 = 08-02 (012_cameras_health.sql + pg_cron healthcheck) → 08-03 (notifications case camera-down/recovered + scheduler last_frame_at wiring) → 08-04 (mediamtx 합성 E2E + RTSP-02 deferred). 수요일 (2026-05-20) D-2.
Last activity: 2026-05-18 — Phase 8 Plan 01 COMPLETE. snapshot.py 에 capture_rtsp (cv2.VideoCapture+CAP_FFMPEG, drift_test.py 검증 패턴) + URL scheme 분기 wrapper + _capture_ffmpeg internal rename. OPENCV_FFMPEG_CAPTURE_OPTIONS module top-level setdefault (Pitfall 6). 6/6 pytest pass + 전체 28/28 무회귀. scheduler.py zero-change (SC #4). 커밋 c3dbf41 · 715c277. 다음 Wave 2 = 08-02 (마이그레이션 + 헬스체크).
