...
Phase 8: ⚠ IN PROGRESS (2026-05-18) — Drift X3 RTSP 실시간 카메라 (RTSP-01·02·03).
...
Status: Phase 8 IN PROGRESS — 08-01·02 ✓ COMPLETE (capture_rtsp + drift_test 패턴 + 012 마이그/cron/healthcheck 운영 DB lock). 다음 = 08-03 (notifications case camera-down/recovered + Edge Function 배포 + 4 curl smoke + scheduler last_frame_at wiring) → 08-04 (mediamtx 합성 E2E + RTSP-02 deferred). 수요일 (2026-05-20) D-2.
Last activity: 2026-05-18 — Phase 8 Plan 02 COMPLETE. supabase/migrations/012_cameras_health.sql + tests/sql/test_012_cameras_health_isolation.sql. supabase db push --linked --yes 성공, vault.create_secret('edge_function_base_url') seeded NOTICE, RPC cameras_healthcheck 204, Pitfall 4 검증 (5 cameras 모두 unknown 유지), T-8-02 anon PATCH 빈 배열 + 후속 GET 보존, A4/A8 testuser1 ↔ cameras 1·5 group_id=1 정합. 커밋 0131ffa · 0755f04. Dashboard 의 service_role_key Vault 시드는 08-03 deploy 전 수동 단계. 다음 Wave 3 = 08-03.
...
- **Phase 8 IN PROGRESS** (Drift X3 RTSP 실시간 카메라, RTSP-01·02·03) — `ai_agent` mp4 →
  RTSP 전환 + 실기기 검증 + 재연결 안정성. Phase 1 cameras 매핑 활용.
  - 08-01 ✓ COMPLETE 2026-05-18 — snapshot.capture_rtsp + URL scheme 분기 wrapper (drift_test 패턴).
  - 08-02·03·04 = pending (Wave 2/3/4).
