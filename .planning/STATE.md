...
- **(해소됨, 2026-05-04) 010_watch_pipeline.sql 운영 DB 적용 완료**:
  - 사용자 승인 후 `supabase db push` 로 적용. 첫 시도에서 `generation expression
    is not immutable` (SQLSTATE 42P17) 오류 → `ts AT TIME ZONE 'UTC'` 캐스트로 fix
    (커밋 `a5dec5f`) 후 재push 성공.
  - 검증 완료 (supabase-py service_role): devices ALTER 적용, 4 신규 테이블 (raw_events,
    wear_state_events, minute_summary, safety_alerts) 생성, RLS 활성, testuser1
    device seed 적재 (device_id=1, MAC 21:02:02:06:01:69, FW 0.6.3.9).
  - migration_repair: 001~008 도 applied 로 마킹 (이전 SQL/Dashboard 적용분 반영).
  - **Wave 2 (04-03) 진입 가능.**
