...
Phase 8: NEW (2026-05-14 추가) — Drift X3 RTSP 실시간 카메라 (RTSP-01·02·03). ai_agent mp4 → RTSP 전환 + 실기기 검증 + 재연결 안정성. **다음 즉시 진입 phase**.
Phase 9: NEW (2026-05-14 추가) — TBM 현장 작업자 가이드 (TBM-01·02·03). 4 신규 테이블 + Android 가이드 화면 + 미참여 알림. 기존 관리자 순회 점검과 동시 운용.
Status: Phase 7 Wave 1·2·3 ✓ COMPLETE (코드 완성). Wave 4 (07-04) DEFERRED — 사용자 PoC 시점 미정. **다음 즉시 = /gsd-discuss-phase 8** (RTSP 실시간 카메라). 수요일 (2026-05-20) D-5.
Last activity: 2026-05-15 — Phase 7 Plan 04 DEFERRED (사용자 시연 환경 부재) + 워치 owner 이전 (testuser1 → test_user, profile + auth user 생성). 다음 phase 8 진입.
