...
created: 2026-04-29
updated: 2026-05-14 (Phase 7 Wave 2 — 07-02 Edge Function watch-ack/pair COMPLETE)
---
...
- [ ] **Phase 8: Drift X3 RTSP 실시간 카메라** — `ai_agent` 가 mp4 대신 RTSP
  스트림 직접 추론, Drift X3 실기기와 연동 + 안정성 (재연결·헬스체크)
...
**Plans**: 4 plans
- [ ] 08-01-PLAN.md — snapshot.py capture_rtsp + URL scheme 분기 wrapper + 6 pytest cases (D-01·02 amended drift_test 패턴, RTSP-01)
- [ ] 08-02-PLAN.md — 012_cameras_health.sql (pg_net + Vault + cameras ALTER + healthcheck 함수 + cron) + [BLOCKING] supabase db push + RLS isolation SQL test (D-03 backend infra, RTSP-03)
- [ ] 08-03-PLAN.md — notifications/index.ts case camera-down + camera-recovered (sendPushToUsers plural, 정정 #5) + deploy + 4 curl smoke + supabase_client.update_camera_health + scheduler 4 detector wiring (RTSP-01·03)
- [ ] 08-04-PLAN.md — mediamtx 다운로드 + scripts/mediamtx.yml + start/stop launcher + cameras 임시 갱신 + scheduler --once-detect E2E + backoff 검증 + 5분 healthcheck round-trip + cameras 원복 (RTSP-01·03 합성 충족, RTSP-02 deferred)
...
| 8. Drift X3 RTSP 실시간 카메라       | 0/0            | Not started   | —          |
