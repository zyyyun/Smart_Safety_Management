...
status: shipped  # 2026-05-22 scope-reduced close — Phase 5·6 v1.1 이월
phases_total: 10
phases_completed: 6  # 1·2·3·8·9·10
...
updated: 2026-05-22 (v1.0 milestone ✓ SHIPPED — scope-reduced close, Phase 5·6 v1.1 이월)
---

# Smart Safety Management — Roadmap (v1.0 5월 PPT 데모 + 수요일 추가) ✓ SHIPPED

> **⚠ ARCHIVED 2026-05-22**: v1.0 milestone 종결 (scope-reduced close). MILESTONES.md 의
> v1.0 Shipped 엔트리 + PROJECT.md Out of Scope (Phase 5·6 v1.1 이월) 참조.
> 본 ROADMAP 은 v1.0 의 phase 정의 + 검증 trace 보존본. v1.1 신규 milestone 시작 시
> `/gsd-new-milestone` 으로 새 ROADMAP 생성.
...
- [x] **Phase 8: Drift X3 RTSP 실시간 카메라** ✓ 2026-05-18 → RTSP-02 실기기 검증 ✓ 2026-05-20 — `ai_agent` 가 mp4 대신 RTSP
  스트림 직접 추론, mediamtx 합성 검증 + 안정성 (재연결·헬스체크) + **Drift X3 (192.168.0.13) 실기기 person 검출 conf=0.92 capture→insert 3.16s**
- [x] **Phase 9: TBM 현장 작업자 가이드** ✓ 2026-05-18 (Plan 04 실기기 1일 사이클 시연 부분 deferred → 사용자 가용 시점) — 4 신규 테이블 + RLS + Realtime publication + tbm-signatures Storage (Option A) + pg_cron + Edge Function 4 case 운영 배포 + Android `tbm/` 패키지 12 main + 2 Activity + ComposeView 임베드 + 48 unit tests PASS + 합성 검증 24/24 PASS (7 isolation + 12 smoke + 5 build/회귀)
...
| 10 | LAN RTSP 자동 수신 감지            | 0    | 4        | Phase 8      | ~0.5~1일      |

**Dependency graph**: 1 → 2 → 3, 4 병렬, 5 ← (1·2·3·4), 6 ← 5, 7 ← 4 (수요일 마감 critical), 8 ← 1, 9 병렬 (시연 흐름은 3·4·7·8 활용), 10 ← 8
...
**Plans**: 4 plans
- [x] 09-01-PLAN.md — 013_tbm_schema.sql (4 테이블 + RLS USING(true) v1.0 + Realtime publication ADD 4 + tbm-signatures Storage 버킷 private Option A + 5 templates 시드 + tbm_missed_attendance_check() SECURITY DEFINER + pg_cron 'tbm_missed_attendance_minute' 1분) + [BLOCKING] supabase db push --linked --yes + scripts/seed_tbm_demo.py (group_id=1 worker 3명) + tests/sql/test_013_tbm_isolation.sql (7 assertions) (TBM-01) — ✓ 2026-05-18 (커밋 f044fac · 20d2c7f, SUMMARY 09-01-SUMMARY.md). Local+Remote=013 migration list 등장 + 5 templates 시드 응답 + tbm_sessions [] + RPC 204 + Storage tbm-signatures public=false + 7/7 assertions PASS + ai_agent 28/28 무회귀. Deviations 2건 (Rule 3: auth.users 트리거 UPSERT 패턴 + psql 미설치 환경 supabase db query 우회).
- [x] 09-02-PLAN.md — notifications/index.ts 에 4 case 추가 (tbm-start / tbm-checkin / tbm-end / tbm-missed, Phase 7 watch-* + Phase 8 camera-* 패턴 union, sendPushToUsers plural 재사용, ownership 검증 T-9-03, leader 검증 T-9-04, UNIQUE 23505 catch Pitfall 5) + supabase functions deploy notifications + 12 curl smoke (4 cases × 3 scenarios) + D-09 회귀 가드 notifications insert 0 (TBM-03) — ✓ 2026-05-18 (커밋 417c203 · aeb6ddf, SUMMARY 09-02-SUMMARY.md, deploy 74.97kB 성공, 12/12 smoke PASS, D-09 delta=0 BEFORE=AFTER=51 검증, T-9-02·03·04·09 mitigation smoke 직접 검증, Pitfall 5·9·11 검증, testuser1 실제 push 도착 notified_count:1 검증, ai_agent/tests 28/28 무회귀). Deviations 1건 (Rule 1: tbm-missed select user_name → name 컬럼명 fix + redeploy).
- [x] 09-03-PLAN.md — Android tbm/ 패키지 신규 (12 main + 4 test 파일, Phase 7 watch/ 1:1 미러 + SignatureCanvas + 2 validator) + storage-kt:2.2.0 의존성 + MyApp.install(Storage) 1줄 + Dynamic session_id 2-stage Realtime 3 채널 + HomeActivity·HomeWorkerActivity ComposeView 임베드 (Pitfall 12 Theme 래핑) + TbmDashboardActivity·TbmWorkerActivity 신규 + MyFirebaseMessagingService tbm_alert 분기 + 4 TDD unit test (WorkTypeValidator + ExpectedEndAtValidator + TbmParticipantsReducer + SignatureState, 21 cases) + compileDebugKotlin BUILD SUCCESSFUL + 회귀 watch/ git diff 0 (TBM-02) — ✓ 2026-05-18 (커밋 c94ee1c · 8df2ced · f4d3f3b · 5f52800 · bfd0cec, SUMMARY 09-03-SUMMARY.md). 48 unit tests PASS (tbm/ 21 + watch/ 26 + Example 1) + ai_agent 28/28 무회귀. Pitfall 1·2·8·12 모두 grep 검증 + T-9-01/06/12/13/14/15 mitigation evidence. Deviations 5건 (모두 Rule 3 plan correction): JUnit4 일관 + UserRole.GENERAL_MANAGER 미존재 + MyApp install order 보존 + SignatureState early-return + JBR PATH 명시.
- [⏸] 09-04-PLAN.md — 1일 사이클 시연 + 캡처 (autonomous: false) — ⏸ DEFERRED (2026-05-18) — 사용자 시연 환경 부재 + `--auto` 모드 directive. Plan 09-01·02·03 합성 검증 24/24 PASS 로 4 SC 코드/인프라 충족 완료. 실측 시연 캡처는 사용자 가용 시점 (5월 PPT 데모 또는 6월 검단·포천 설치 직전) 으로 이연 (Phase 7-04 + Phase 8 RTSP-02 패턴 1:1 미러). 09-04-SUMMARY.md `status: deferred` (TBM-01·02·03 검증)

### Phase 10: LAN RTSP 자동 수신 감지
**Goal**: PC와 Drift X3 카메라가 같은 네트워크에 있다는 전제에서, 사용자가 본부/현장
PC에서 별도 demo/pull 스크립트를 실행하지 않아도 Windows service 로 떠 있는
`ai_agent/main.py` 가 등록된 RTSP 카메라의 수신 가능 상태를 자동 감지하고 기존 YOLO
detection cycle 을 즉시 구동한다.
**Depends on**: Phase 8 — Drift X3 RTSP URL, `cameras` 매핑, YOLO/FCM 파이프라인 검증값.
**Requirements**: 없음 — v1.0 제품 요구사항이 아니라 운영 자동화/architecture evidence phase.
**Success Criteria** (what must be TRUE):
  1. `SmartSafetyAiAgent` service 가 상시 실행 중이면 등록된 RTSP URL 카메라의
     host:port 수신 가능 상태를 주기적으로 probe 한다.
  2. `unknown/unreachable → reachable` 전이에서 별도 사용자 스크립트 없이 기존
     `run_detection_cycle()` 을 즉시 트리거한다.
  3. 기존 1분 `general_detection` job 과 자동 감지 trigger 가 겹쳐도 중복 실행 충돌이 없다.
  4. Drift X3 가 같은 LAN 에 붙은 뒤 1분 내 `detection_events` 에 person 등 YOLO 이벤트가
     적재되거나, no_detect 로그로 cycle 구동 사실이 확인된다.
**Plans**: 1 plan
- [x] 10-01-PLAN.md — RTSP TCP probe + scheduler auto-detect job + service 운영 시나리오 +
  단위 테스트 — ✓ 2026-05-22 (SUMMARY 10-01-SUMMARY.md, ai_agent 31/31 PASS)
...
**Mapped**: 28/28 ✓
**Architecture evidence phases**: Phase 10 (0 product REQs, 4 auto-detect criteria)
**Orphans**: 0
**Duplicates**: 0
...
| 8. Drift X3 RTSP 실시간 카메라       | 4/4            | COMPLETE ✓ (RTSP-02 실기기 검증 ✓ 2026-05-20) | 2026-05-18 / RTSP-02 2026-05-20 |
| 9. TBM 현장 작업자 가이드            | 3/4            | COMPLETE ✓ (Plan 04 1일 시연 deferred) | 2026-05-18 |
| 10. LAN RTSP 자동 수신 감지          | 1/1            | COMPLETE ✓    | 2026-05-22 |
