...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      // ????????????????????????????????????????????????????????????????????????????????????
      // WATCH_ALERT ??J2208A ???뒄 ?袁る퓮 ???뵝 (per CONTEXT.md D-11夷똃-12)
      // ????????????????????????????????????????????????????????????????????????????????????
      // BLE ?????곷섧??(j2208a/supabase_writer.py:call_watch_alert_edge_function)
      // 揶쎛 service_role 嚥??紐꾪뀱. payload ?④쑴鍮?? 域밸챷????λ땾?? 1:1 筌띲끉臾?
      // alert_type / severity ?遺우뵠?紐꺿봺??쎈뱜??010_watch_pipeline.sql ??      // safety_alerts CHECK constraint ?? ??덉뵬 ??Edge Function ????甕???野꺜筌?      // (T-04-12 STRIDE: ?醫듚?野껋럡????tampering 筌△뫀??.
      // 'NORMAL' severity ??resolution ???뵝 ?袁⑹뒠 ??DB ?紐꾧퐣?紐껊뮉 ??揶쎛筌왖筌?      // FCM payload data ?袁⑤굡嚥??袁⑤뼎??뤿선 ?????곷섧?硫? ??깃맒/?袁⑹뵠???브쑨由?揶쎛??
      case "watch-alert": {
        ...
        // ???뵝 ?紐껋쟿??곸뒠 DB ?紐꾧퐣??(UI ???뵝 筌뤴뫖以???뽯뻻 ??疫꿸퀣??send_individual ???쉘????덉뵬)
        const { error: insErr } = await supabase
          .from("notifications")
      ...
      // ????????????????????????????????????????????????????????????????????????????????????
      // SafetyAlertsActivity ??acknowledge 甕곌쑵????癰?Edge Function ?紐꾪뀱.
      // SQL: UPDATE safety_alerts SET ack_at=now() WHERE alert_id=$1
      //      AND device_id IN (SELECT device_id FROM devices WHERE user_id=$2)
      //      AND ack_at IS NULL
      // T-7-02 mitigation (cross-worker tampering): ownership 野꺜筌앹빘? device_id IN (...)
      // T-7-05 mitigation (clock spoofing): ??뺤쒔筌?new Date().toISOString() ??client ??볧??얜똻??
      // D-09 ???뵝 ?袁⑹뵠 ?癒?뒅: ack ?癒?퍥???????뵝 獄쏆뮇源?X (notifications insert / FCM 獄쏆뮇??????
      // idempotency: .is('ack_at', null) 揶쎛??????甕곕뜆??ack ??0 rows + 404 "already acknowledged"
      // ?뚎됱쓥筌? ack_at (010_watch_pipeline.sql) ??REQUIREMENTS.md 吏? ??acknowledg* ??볥┛????븍┛ (Pitfall 5)
      case "watch-ack": {
        ...
        // ownership 野꺜筌?(T-7-02): 癰?user_id 揶쎛 癰귣똻???devices ??alert 筌?ack 揶쎛??        const { data: ownDevices, error: devErr } = await supabase
          .from("devices")
          .select("device_id")
          .eq("user_id", user_id);
        ...
        // ack_at 揶쏄퉮????server-side toISOString() + idempotency 揶쎛??        const nowIso = new Date().toISOString();
        const { data, error } = await supabase
          .from("safety_alerts")
      ...
      // ????????????????????????????????????????????????????????????????????????????????????
      // SettingDeviceManagementActivity ??J2208A ???뒄 ?諭????癰?Edge Function ?紐꾪뀱.
      // op='pair'   : payload {action, op, user_id, mac_address}
      //   - MAC ?類ㅻ뻼 ???筌?(T-7-03 client validation ?怨좎돳 筌△뫀??
      //   - 疫꿸퀣??device row ??user_id 揶쎛 NULL ??욧탢??癰?user_id ?????춸 ??됱뒠
      //   - ??삘뀲 user 揶쎛 paired ?????뒄 ??409 (???뒄 揶쎛嚥≪뮇肄덃묾?筌△뫀??
      //   - 揶쏆늿? user ??揶쏆늿? mac ??踰묉에???idempotent UPDATE
      //   - ??뺣굡 ??MAC ??INSERT (device_type='WATCH', serial_number='J2208A-{MAC}')
      // op='unpair' : payload {action, op, user_id}
      //   - 癰귣챷??user_id + device_type='WATCH' ??mac_address NULL ??      //
      // T-7-03 mitigation: WHERE (user_id IS NULL OR user_id=$2) ????삘뀲 worker 揶쎛
      // ??? paired ?????뒄??selectErr ??409 ?臾먮뼗. MAC ?類?뇣??(???얜챷???類?뇣??
      // ??곗쨮 client validation ?怨좎돳 ??뺣즲 筌△뫀??
      case "watch-pair": {
        ...
        if (op === "pair") {
          ...
          // ownership 野꺜筌?(T-7-03 spoofing 筌△뫀??: mac ??device 揶쎛 (a) 沃섎챸釉??or (b) 癰귣챷??癰귣똻?
          ...
          if (existing) {
            // idempotent UPDATE (揶쏆늿? user ??揶쏆늿? mac ??踰묉에?OK)
          ...
          // unpair ??re-pair ?냈??곷뮞: serial_number ???醫????筌?mac ?? NULL.
          // serial_number = `J2208A-{MAC}` 嚥?????? ??삘뀲 user 揶쎛 癰귣똻? ??409.
          ...
          if (bySerial) {
            ...
            // mac_address + user_id ????癰귣벀??(unpair 揶쎛 mac 筌?NULL ??됰선?? ?癒?뮉 ??삘뀲 癰궰??
            const { error: updErr } = await supabase
              .from("devices")
          ...
          // ?醫됲뇣 device INSERT (010 ??뺣굡 ??MAC, 筌ｌ꼷???源낆쨯)
          ...
          // op === "unpair" ??癰귣챷?????뒄筌?mac_address NULL
          const { data, error } = await supabase
            .from("devices")
      ...
      // ????????????????????????????????????????????????????????????????????????????????????
      // CAMERA-DOWN ??Phase 8 RTSP-03 (per 08-CONTEXT.md D-03 / 08-RESEARCH 吏쟑attern 4)
      // ????????????????????????????????????????????????????????????????????????????????????
      // pg_cron (012_cameras_health.sql cameras_healthcheck()) 揶쎛 5???얜똻???燁삳?李??곗춳??      // service_role JWT 嚥??紐꾪뀱. cron ?癒?퍥揶쎛 30??cooldown + last_alert_at + ?怨밴묶 ?袁⑹뵠
      // 筌?굞??(D-09 ???뵝 ?袁⑹뵠 ?癒?뒅) ??癰??냈??곷뮞??push-only. notifications insert ??곸벉
      // (??? 揶쎛?? 癰?case ?紐꾪뀱嚥?notifications row 筌앹빓? 0).
      //
      // C5 ?類ㅼ젟 (RESEARCH ?類ㅼ젟 #5): manager 亦낅슦釉??????N筌???sendPushToUsers (plural).
      // Phase 4 watch-alert (single-recipient) ???쉘 癰귣벊沅?疫뀀뜆?.
      //
      // C4 ?類ㅼ젟 (RESEARCH ?類ㅼ젟 #4 / Pitfall 3): Option B (deadline ?怨쀪퐨) ??Android
      // channel_id 筌뤿굞??X. fcm_default_channel ??沅??(Android ?꾨뗀諭?癰궰野?0). v1.1
      // ?癒?퐣 'camera_alerts' 筌?쑬瑗??브쑬??
      case "camera-down": {
        ...
        const r = await sendPushToUsers(supabase, userIds, {
          title: "燁삳?李?????뻿?癒?쟿",
          body: `${camName} (${camArea}) 5????곴맒 frame ?얜똻???,
          data: {
            type: "camera_alert",
      ...
      // ????????????????????????????????????????????????????????????????????????????????????
      // CAMERA-RECOVERED ??Phase 8 RTSP-03 ???궗 ???뵝 (D-09 ?ル굝利????뵝 ???쉘)
      // ????????????????????????????????????????????????????????????????????????????????????
      // down?萸쩵 ?袁⑹뵠 ??뽰젎??cron ??λ땾揶쎛 1???紐꾪뀱. camera-down ????????용뮞?紐껋춸 癰궰野?
      // last_frame_at ?? ???궗 ???뵝???븍뜇釉??(獄쎻뫕??frame ??뤿뻿??됱벉 = recover trigger).
      case "camera-recovered": {
        ...
        const r = await sendPushToUsers(supabase, userIds, {
          title: "燁삳?李?????궗",
          body: `${camName} (${camArea}) frame ??뤿뻿 ??而?,
          data: {
            type: "camera_alert",
            camera_id: String(camera_id),
            severity: "NORMAL",
      ...
      // ????????????????????????????????????????????????????????????????????????????????????
      // TBM-START ??Phase 9 TBM-03 (per 09-CONTEXT.md D-08 / 09-RESEARCH 吏쟑attern 4)
      // ????????????????????????????????????????????????????????????????????????????????????
      // ?온?귐딆쁽揶쎛 ??삳뮎 TBM ?紐꾨????뽰삂. 4 ??ｍ?
      //  (1) tbm_templates 嚥≪뮆???work_type 癰?checklist 鈺곌퀬??
      //  (2) tbm_sessions INSERT ??UNIQUE (group_id, session_date) ?겸뫖猷???23505 ??409
      //      (T-9-02 mitigation, Pitfall 5 ??곕돗 ??DB-level UNIQUE 揶쎛 ?醫듚?野껋럡??
      //  (3) tbm_checklists ??template.checklist ??揶?????snapshot bulk INSERT
      //      (JSONB array order 癰귣똻????Pitfall 11)
      //  (4) group ??worker N筌?(leader ??뽰뇚) ?癒?쓺 sendPushToUsers (plural) ??Phase 8 ???쉘
      //
      // D-09 ??? 揶쎛?? notifications ???뵠??insert ??곸벉 (push-only). ?怨밴묶 ?袁⑹뵠 筌?굞???
      // tbm_sessions row ?癒?퍥 + tbm_participants insert (worker checkin) + missed_alert_at
      // (cron) ??揶쎛筌? 癰?case ?紐꾪뀱嚥?public.notifications row 筌앹빓? 0.
      case "tbm-start": {
        const {
          leader_user_id,
          group_id,
