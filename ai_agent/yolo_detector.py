...
class GenericYoloDetector:
    ...
    def __init__(
        ...
        log.info(
            "GenericYoloDetector loaded name=%s framework=%s weights=%s target_classes=%s model_class_count=%d (%.0fms)",
            name,
            framework,
            weights.name if is_local_path else weights_path,
            sorted(self.targets) if self.targets is not None else "all",
