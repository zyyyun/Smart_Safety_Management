...
#                 tbm_checklists 5 rows INSERT (electric work_type, 5 항목)
...
#   1. 정상: leader=testuser1, group_id=1, work_type=electric, expected_end_at=now+15min
#      → HTTP 200 + ok:true + session_id + checklist_count:5
...
echo "=== Test 1: 정상 (leader=testuser1, group_id=1, work_type=electric) ==="
...
grep -q '"checklist_count":5' /tmp/tbm_start_1.json || { echo "EXPECTED checklist_count:5" >&2; exit 4; }
...
grep -q '이미 오늘 세션' /tmp/tbm_start_2.json || { echo "EXPECTED '이미 오늘 세션' message" >&2; exit 7; }
PASS=$((PASS+1))

echo "=== Test 3: payload 누락 → 400 (expected_end_at 생략) ==="
HTTP=$(curl -sS -o /tmp/tbm_start_3.json -w "%{http_code}" -X POST "$URL" \
    -H "apikey: $SUPABASE_ANON_KEY" -H "Authorization: Bearer $SUPABASE_ANON_KEY" \
    -H "Content-Type: application/json" \
    -d '{"action":"tbm-start","leader_user_id":"testuser1","group_id":1,"work_type":"electric"}')
echo "  HTTP $HTTP"; head -c 500 /tmp/tbm_start_3.json; echo
[[ "$HTTP" == "400" ]] || { echo "EXPECTED 400" >&2; exit 8; }
grep -q 'required' /tmp/tbm_start_3.json || { echo "EXPECTED required-fields message" >&2; exit 9; }
PASS=$((PASS+1))

echo "=== tbm-start: ${PASS}/3 PASS ==="
echo "SESSION_ID=$SESSION_ID"
